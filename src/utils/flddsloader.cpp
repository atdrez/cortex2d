...
bool FlDDSTexture::loadFromFile(const char *fileName)
{
    clear();
...
bool FlDDSTexture::readTexture(FlFile &fp)
{
    /*******************************************
     * DDS HEADER FORMAT
     *
    ...
    const fluint32 magic = header.readUInt32(0);
    ...
    const fluint32 dwSize = header.readUInt32(4);
    ...
    bufferSize = flMax(divSize, width) / divSize * flMax(divSize, height) / divSize * blockBytes;
