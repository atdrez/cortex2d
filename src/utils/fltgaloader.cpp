...
bool FlTGATexture::loadFromFile(const char *fileName)
{
    clear();
...
bool FlTGATexture::readHeader(FlFile &fp, bool *compressed)
{
    /*******************************************
     * TGA HEADER FORMAT
     *
    ...
    *******************************************/
    ...
    flubyte idLength = header.readUByte(0);
    flubyte colorMapType = header.readUByte(1);
    flubyte dataTypeCode = header.readUByte(2);
    ...
    fp.seek(idLength, FlFile::CurSeek);
...
bool FlTGATexture::loadFileData(FlFile &fp, bool compressed)
{
    if (!compressed) {
        ...
        for (fluint32 i = 0; i < bufferSize; i += bytesPerPixel)
            convertTGAPixel(&buffer[i], bytesPerPixel);

        return true;
    }

    for (fluint32 i = 0; i < bufferSize; ) {
        flubyte packetHeader;
        ...
        fluint32 packetCount = (packetHeader & 127) + 1; // 7-bits (number of packets)
        fluint32 packetFinalPos = (i + bytesPerPixel * packetCount);
